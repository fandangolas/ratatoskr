defmodule Ratatoskr.UseCases.PublishPartitionedMessage do
  ...
  @type result :: {:ok, message_id :: String.t(), partition_id :: non_neg_integer()} | {:error, reason :: atom()}
    ...
    case is_partitioned_topic?(topic_name, deps) do
      ...
      false ->
        ...
        case PublishMessage.execute(topic_name, payload, opts, deps) do
          {:ok, message_id} -> {:ok, message_id, 0}  # Single partition = partition 0
          error -> error
        end
    end
  end
              ...
              case Ratatoskr.UseCases.PublishMessageBatch.execute(fallback_batch_messages, deps) do
                {:ok, batch_results} ->
                  Enum.map(batch_results, fn result ->
                    %{result | partition_id: 0}  # Add partition_id for consistency
                  end)
  ...
  defp publish_batch_to_partitioned_topic(topic_name, messages, _deps) do
    ...
    processed_messages = 
      Enum.map(messages, fn msg ->
        ...
        case Message.new(topic_name, msg.payload, metadata: metadata, partition_key: partition_key) do
          {:ok, message} -> 
            %{
              message: message,
  ...
  defp emit_partitioned_metrics(message, partition_id, %{metrics: metrics}) when not is_nil(metrics) do
  ...
  defp publish_partitioned_domain_event(message, partition_id, %{event_publisher: publisher}) when not is_nil(publisher) do
    event = 
