defmodule Ratatoskr.BenchmarkHelpers do
  ...
  def measure_latencies(message_count, topic_name) do
    ...
    latencies = for i <- 1..message_count do
      ...
      case Ratatoskr.publish(topic_name, %{id: i, start_time: start_time}) do
        {:ok, _} ->
          ...
          after 
            5000 -> :timeout  # 5 second timeout per message
          end
    ...
    if length(valid_latencies) < message_count do
      Logger.warning("Only #{length(valid_latencies)}/#{message_count} messages completed successfully")
    end
  ...
  """
  @spec test_concurrent_subscribers(integer(), String.t(), integer()) :: {:ok, map()} | {:error, term()}
  ...
  defp calculate_percentiles(latencies) do
    ...
    %{
      count: len,
      min: (Enum.min(sorted) / 1000) |> Float.round(3),     # Convert μs to ms
      max: (Enum.max(sorted) / 1000) |> Float.round(3),     # Convert μs to ms  
      p50: percentile(sorted, 0.50) |> Float.round(3),      # Median
      p95: percentile(sorted, 0.95) |> Float.round(3),
      p99: percentile(sorted, 0.99) |> Float.round(3),
      avg: (Enum.sum(sorted) / len / 1000) |> Float.round(3)  # Average
    }
  end

  defp percentile(sorted_list, percentile) do
    len = length(sorted_list)
    index = max(1, round(percentile * len)) - 1  # Convert to 0-based index
    (Enum.at(sorted_list, index) / 1000)  # Convert μs to ms
  end
  ...
  defp collect_responses(responses, remaining, timeout_ms) do
    ...
    after 
      timeout_ms ->
        responses  # Return what we have so far
    end
  end
end