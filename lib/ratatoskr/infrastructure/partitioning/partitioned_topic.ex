defmodule Ratatoskr.Infrastructure.Partitioning.PartitionedTopic do
  ...
  def init(opts) do
    ...
    case PartitionManager.start_link(partition_manager_opts) do
      ...
      {:error, reason} ->
        Logger.error("Failed to start partition manager for topic #{topic_name}: #{inspect(reason)}")
        {:stop, reason}
    end
  end
  ...
  def handle_call({:publish_batch, messages}, _from, state) when is_list(messages) do
    Logger.debug("Publishing batch of #{length(messages)} messages to partitioned topic: #{state.topic_name}")
  ...
  defp update_topic_stats(stats, partition_id, message) do
    ...
    %{stats |
      ...
      partition_message_counts: Map.put(stats.partition_message_counts, partition_id, current_partition_count + 1),
  ...
  defp extract_partition_key_from_batch_message(message) do
    ...
    cond do
      ...
      is_map(message) and Map.has_key?(message, :metadata) and Map.has_key?(message.metadata, :partition_key) ->
