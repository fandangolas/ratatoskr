defmodule Ratatoskr.Infrastructure.Partitioning.PartitionManager do
  ...
  def init(opts) do
    ...
    Logger.info("Starting partition manager for topic: #{topic_name} with #{partition_count} partitions")
  ...
  def handle_call({:route_message, message, partition_key}, _from, state) do
    ...
    partition_id = calculate_partition(message, partition_key, state.partition_count, state.hash_ring)
      ...
      partition_server ->
        ...
        case GenServer.call(partition_server, {:publish, message}) do
          {:ok, message_id} ->
            ...
            new_assignments = update_partition_stats(state.partition_assignments, partition_id, message)
  ...
  def handle_call(:get_partition_info, _from, state) do
    partition_info = %{
      ...
      partitions: Enum.map(0..(state.partition_count - 1), fn partition_id ->
  ...
  def handle_call(:get_partition_stats, _from, state) do
    stats = %{
      ...
      total_messages: 
        ...
        |> Enum.sum(),
      partition_distribution: Enum.into(state.partition_assignments, %{}, fn {partition_id, assignment} ->
        {partition_id, %{
          message_count: assignment.message_count,
          last_message_at: assignment.last_message_at
        }}
    ...
    partition_servers = 
      ...
      |> Enum.reduce(%{}, fn partition_id, acc ->
        ...
        case start_partition_server(partition_name, topic_name, partition_id) do
          ...
          {:error, reason} ->
            Logger.error("Failed to start partition server #{partition_id} for topic #{topic_name}: #{inspect(reason)}")
            acc
        end
      end)
