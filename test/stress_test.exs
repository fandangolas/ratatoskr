defmodule Ratatoskr.StressTest do
  ...
  @moduletag :stress
  @moduletag timeout: 60_000  # 60 second timeout for stress tests
  ...
  describe "Concurrency Stress Tests" do
    test "supports 100 concurrent subscribers" do
      ...
      assert {:ok, results} = BenchmarkHelpers.test_concurrent_subscribers(100, topic_name, 10_000)
    ...
    test "concurrent publishers to single topic" do
      ...
      {time_ms, publisher_pids} = :timer.tc(fn ->
        for i <- 1..publisher_count do
          spawn_link(fn ->
            for j <- 1..messages_per_publisher do
              {:ok, _} = Ratatoskr.publish(topic_name, %{
                publisher: i, 
                message: j, 
    ...
    test "multiple topics with concurrent activity" do
      ...
      Logger.info("Starting multi-topic stress test: #{topic_count} topics, #{subscribers_per_topic} subscribers each")
  ...
  describe "Resource Management" do
    test "handles subscriber churn (connects/disconnects)" do
      ...
      for cycle <- 1..churn_cycles do
        # Connect subscribers
        subscribers = for i <- 1..subscribers_per_cycle do
          spawn_link(fn ->
  ...
  defp collect_messages(messages, remaining, timeout) do
    ...
    after
      timeout ->
        messages  # Return what we collected
    end
  end
